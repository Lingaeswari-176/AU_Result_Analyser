...
SUBJECT_NAMES = {
    ...
    "NM1134":
        "Foundation Skills For Employability"
}
...
if not st.session_state.parsed_result:
    ...
    # Native Streamlit components avoid showing HTML as literal text.
    st.title("🎓 Anna University Result Analyzer")
    st.caption("Semester 4 • 2024–2028 CSE • Automatic GPA • Section-wise Analysis")
...
st.title("Semester 4 Result Analytics")
st.caption("2024–2028 CSE  |  GPA-only analysis  |  Register-wise ranking  |  Section-wise reports")
...
c1.metric("TOTAL 2024 BATCH", len(students))
c2.metric("SECTION A", section_counts.get("A", 0))
c3.metric("SECTION B", section_counts.get("B", 0))
c4.metric("SECTION C", section_counts.get("C", 0))
...
for tab, section in [
    ...
    with tab:
        ...
        st.subheader(f"Semester 4 — Section {section}")
        st.caption(
            f"{analysis['total']} students | GPA calculated automatically"
        )
